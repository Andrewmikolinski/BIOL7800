...
test_that("Error returned when working with non-character or non-numeric input input", {
  expect_error(less_mlatlongdf(1:5))
})
test_that("It should work with character and numeric input", {
  expect_equal(less_mlatlongdf(EriL1, 33.288275, -82.899328, 35.197685, -82.226109, "South Carolina"), EriL1[EriL1$decimalLatitude > 33.288275 & EriL1$decimalLatitude < 35.197685 & EriL1$decimalLatitude < ((m * EriL1$decimalLongitude) + int) & EriL1$decimalLongitude > ((EriL1$decimalLatitude - int)/m) & EriL1$stateProvince == "South Carolina", ])
})
